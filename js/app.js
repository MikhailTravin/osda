(() => {
    ...
    if (document.querySelector(".images-product__slider-popup")) {
    ...
    scrollBlock();
    const buttons = document.querySelectorAll(".main-home__item");
    if (buttons) buttons.forEach((button => {
        const mainHome = document.querySelector(".main-home");
        const closes = document.querySelectorAll(".main-content__close");
        const id = button.dataset.id;
        const contentSelector = `div[data-id='${id}'].main-content__content`;
        const contentBlock = document.querySelector(contentSelector);
        const mainContents = document.querySelectorAll(".main-content__content");
        const buttonBasket = document.querySelector(".button-basket");
        const contentBasket = document.querySelector(".main-content__content.main-content-basket");
        buttonBasket.addEventListener("click", (function(e) {
            mainContents.forEach((content => {
                content.classList.remove("_active");
            }));
            if (document.querySelector(".main-content__content._active")) {
                mainHome.classList.add("_active");
                document.documentElement.classList.add("_lock");
            }
            contentBasket.classList.add("_active");
        }));
        ...
        closes.forEach((close => {
            close.addEventListener("click", (function(e) {
                ...
                contentBasket.classList.remove("_active");
                mainHome.classList.remove("_active");
                document.documentElement.classList.remove("_lock");
            }));
        }));
        window.addEventListener("click", (e => {
            const target = e.target;
            if (!target.closest(".main-home__item") && !target.closest(".main-content__content") && !target.closest(".popup")) {
                mainContents.forEach((content => {
                    content.classList.remove("_active");
                }));
                mainHome.classList.remove("_active");
    ...
    const mainScrolls = document.querySelectorAll(".main-content__content");
    if (mainScrolls) {
        const header = document.querySelector(".header");
        mainScrolls.forEach((scroll => {
            scroll.addEventListener("wheel", (function(e) {
                const course = e.wheelDelta;
                if (course > 0) header.classList.remove("scroll"); else header.classList.add("scroll");
            }));
        }));
    }
    const buttonsDownload = document.querySelectorAll(".button-download__button");
    if (buttonsDownload) {
        const buttonContents = document.querySelectorAll(".button-download__content");
        const closes = document.querySelectorAll(".button-download__close");
        buttonsDownload.forEach((button => {
            ...
            window.addEventListener("click", (e => {
                const target = e.target;
                if (!target.closest(".button-download__content") && !target.closest(".button-download__button")) {
                    button.classList.remove("_active");
                    buttonContents.forEach((content => {
                        content.classList.remove("_active");
                    }));
    ...
    let body = document.querySelector(".main-home__content");
    if (body) {
        let titles = document.querySelectorAll(".main-home__titles");
        let mainHome = document.querySelector(".page__main-home");
        const mainHomeArrows = document.querySelectorAll(".main-home-arrow");
        const mainHomeLink = document.querySelector(".main-home__link");
        document.querySelector(".main-home__arrows");
        function start() {
            titles.forEach((title => {
                let hBody = window.getComputedStyle(body, false).height;
                hBody = Number(hBody.slice(0, hBody.length - 2));
                let hLink = window.getComputedStyle(mainHomeLink, false).width;
                hLink = Number(hLink.slice(0, hLink.length - 2));
                let hTitle = window.getComputedStyle(title, false).height;
                hTitle = Number(hTitle.slice(0, hTitle.length - 2));
                title.style.bottom = hBody + "px";
                if (document.documentElement.clientWidth > 1560) title.style.left = hLink + "px"; else title.style.left = "0";
                mainHomeArrows.forEach((arrow => {
                    arrow.style.bottom = hBody + "px";
                }));
                if (document.documentElement.clientWidth < 991.98) mainHome.style.minHeight = `calc(100vh + ${hTitle}px)`;
            }));
        }
